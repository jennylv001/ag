...
async def main():
    ...
    try:
        ...
        agent_settings = AgentSettings(
            task=(
                '''“Load oregonbuys.gov → Advanced Search → Select Blankets from the Dropdown, then wait a second, select ‘Department of Administration’ from the Organization dropdown in the new fields shown, run search → iterate results: open each, extract purchaser/contractor names+emails, return → save all to a separate sheets.google.com tab, validate completeness, save the page when done, aggregate 200 pairs..”'''
            ),
            llm=llm_manager,
