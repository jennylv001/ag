...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	advanced_stealth: bool = Field(
		default=False,
		description=(
			'Enable advanced behavioral stealth: entropy-driven timing and motion, interaction planning, page exploration, '
			'error simulation, navigation, typing, and scrolling. When True, all advanced behaviors are enabled. '
			'When only stealth=True, core behaviors (typing and scrolling) are enabled. '
			'(Note: advanced_stealth implies stealth; if advanced_stealth=True and stealth=False, stealth will be enabled.)'
		)
	)
	...
	# Do not mutate the live DOM with highlight overlays by default
	highlight_elements: bool = Field(default=False, description='Highlight interactive elements on the page (adds temporary overlays).')
	# Server-side screenshot overlays (no DOM mutation). Off by default; useful for debugging.
	overlay_highlights_on_screenshots: bool = Field(
		default=True,
		description='Composite clickable element highlight boxes onto screenshots server-side (no DOM mutation).',
	)
	# Limit number of items drawn to avoid clutter and overhead
	overlay_max_items: int = Field(default=60, description='Maximum number of highlighted elements to draw on screenshots.')
	...
	@model_validator(mode='after')
	def ensure_advanced_stealth_implies_stealth(self) -> Self:
		"""Ensure that advanced_stealth turns on stealth automatically.

		This avoids surprising configurations where advanced behaviors are requested
		but core stealth is disabled. It keeps the feature map consistent with
		BrowserSession._compute_stealth_features().
		"""
		try:
			if self.advanced_stealth and not self.stealth:
				self.stealth = True
		except Exception:
			pass
		return self
	...
	def apply_stealth_defaults(self) -> Self:
		...
		try:
			if self.stealth:
				...
				# IMPORTANT: only assign when None to avoid triggering endless revalidation loops
				if self.locale is None:
					self.locale = 'en-US'
				# Prefer headful mode for stealth unless user explicitly set headless
				if self.headless is None:
					self.headless = False
				# Prefer window (no viewport) behavior in headful for realistic rendering unless explicitly set
				if self.no_viewport is None:
	...
	def warn_user_data_dir_non_default_version(self) -> Self:
		...
		# In stealth mode, when no explicit custom user_data_dir is provided, we prefer ephemeral contexts.
		# Avoid rewriting the default dir name to default-<channel> to prevent confusion and accidental persistence.
		if (
			self.user_data_dir == CONFIG.BROWSER_USE_DEFAULT_USER_DATA_DIR
			and is_not_using_default_chromium
			and not self.stealth
		):
			alternate_name = (
			...
			logger.warning(
				f'⚠️ {self} Changing user_data_dir= {_log_pretty_path(self.user_data_dir)} ➡️ .../default-{alternate_name} to avoid {alternate_name.upper()} corrupting default profile created by {BROWSERUSE_DEFAULT_CHANNEL.name}'
			)
	...
	def get_args(self) -> list[str]:
		...
		# Capture args before conversion. Place user-provided args LAST so they take precedence
		# when deduping/merging (dict wins with the last value for a given key).
		pre_conversion_args = [
			*default_args,
			...
			*(self._get_extension_args() if self.enable_default_extensions else []),
			# User args last to allow explicit override
			*self.args,
		]

		# Strict stealth filtering: remove automation-revealing flags at the source when stealth=True
		# Goal: minimize command-line fingerprints that anti-bot systems look for.
		if self.stealth:
			# Exact-match flags to strip entirely when running in stealth mode
			STEALTH_STRIP_EXACT: set[str] = {
				'--disable-background-networking',
				'--metrics-recording-only',
				'--no-first-run',
				'--password-store=basic',
				...
				'--test-type=gpu',
				'--disable-sync',
				# In strict stealth, also drop explicit NetworkService enablement to reduce automation fingerprints
				'--enable-features=NetworkService,NetworkServiceInProcess',
				'--disable-domain-reliability',
				'--disable-datasaver-prompt',
				'--safebrowsing-disable-auto-update',
				...
				'--install-autogenerated-theme=0,0,0',
				'--disable-focus-on-load',
				'--disable-window-activation',  # already removed in headful flows above; keep here for completeness
				'--simulate-outdated-no-au="Tue, 31 Dec 2099 23:59:59 GMT"',
				'--hide-crash-restore-bubble',
				'--ash-no-nudges',
				'--no-pings',
			}
			# Prefix-based strips (remove broad groups of toggles typical of automation/test envs)
			# Keep NetworkService features enabled in stealth for parity with non-stealth behavior.
			STEALTH_STRIP_PREFIXES: tuple[str, ...] = (
				'--disable-features=',  # remove the large disable-features bundle including AutomationControlled
			)

			filtered_args: list[str] = []
			for _arg in pre_conversion_args:
				if _arg in STEALTH_STRIP_EXACT:
					continue
				if any(_arg.startswith(p) for p in STEALTH_STRIP_PREFIXES):
					continue
				filtered_args.append(_arg)

			# Important: keep AutomationControlled disabled even in stealth to avoid easy detection.
			# We intentionally add the Blink-specific flag so that even if we prune broader --disable-features,
			# the critical automation fingerprint remains off.
			if not any(a.startswith('--disable-blink-features=') for a in filtered_args):
				filtered_args.append('--disable-blink-features=AutomationControlled')

			pre_conversion_args = filtered_args

		# Always strip known-problematic/unsupported flags regardless of mode
		# Chrome shows a warning banner for '--extensions-on-chrome-urls' and it provides no benefit for automation.
		UNSUPPORTED_FLAGS: set[str] = {
			'--extensions-on-chrome-urls',
		}
		pre_conversion_args = [a for a in pre_conversion_args if a not in UNSUPPORTED_FLAGS]
	...
	def _get_extension_args(self) -> list[str]:
		...
		Note: Chrome extensions are not reliably supported in headless mode; skip to avoid stalls.
		"""
		try:
			if self.headless:
				logger.info('🧩 Skipping default extensions: headless mode detected')
				return []
		except Exception:
			# If state is not yet fully initialized, fall through safely
			pass
	...
	def _download_extension(self, url: str, output_path: Path) -> None:
		...
		try:
			# Add a short timeout to avoid long stalls on networks blocking client2.google.com
	...
	def kwargs_for_launch_persistent_context(self) -> BrowserLaunchPersistentContextArgs:
		"""Return kwargs for BrowserType.launch_persistent_context()."""
	...
	def kwargs_for_launch(self) -> BrowserLaunchArgs:
		"""Return kwargs for BrowserType.launch()."""
